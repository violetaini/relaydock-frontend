...
function SettingsPage({ notify }: { notify: (message: string, tone?: ToastState["tone"]) => void }) {
  const [masterURL, setMasterURL] = useState(""); const [interval, setIntervalValue] = useState(5000); const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [error, setError] = useState("");
  useEffect(() => { Promise.all([api.get<{ master_url: string }>("/api/admin/system-settings/master-url"), api.get<{ refetch_interval_ms: number }>("/api/system-config/refetch-interval")]).then(([master, refresh]) => { setMasterURL(master.master_url || location.origin); setIntervalValue(refresh.refetch_interval_ms || 5000); }).catch((reason) => setError(reason instanceof Error ? reason.message : "设置加载失败")).finally(() => setLoading(false)); }, []);
