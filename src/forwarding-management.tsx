...
function defaultGrantDraft(users: UserItem[], templates: TunnelTemplate[]): GrantDraft {
  return {
    username: users.find((user) => user.role !== "admin")?.username || users[0]?.username || "",
...
function TemplateDialog({ servers, onClose, onComplete }: { servers: RemoteServer[]; onClose: () => void; onComplete: () => Promise<void> }) {
  ...
  const validPorts = Number(draft.portStart) >= 39000 && Number(draft.portEnd) <= 40000 && Number(draft.portStart) <= Number(draft.portEnd);
...
function GrantDialog({ grant, users, templates, onClose, onComplete }: { grant?: TunnelGrant; users: UserItem[]; templates: TunnelTemplate[]; onClose: () => void; onComplete: () => Promise<void> }) {
  ...
  const valid = draft.username && draft.tunnelID && draft.startsAt && Number(draft.maxForwards) >= 1 && Number(draft.speedMbps) >= 0 && Number(draft.connectionLimit) >= 0 && Number(draft.trafficGB) >= 0 && (!draft.expiresAt || new Date(draft.expiresAt).getTime() > new Date(draft.startsAt).getTime());
  const submit = async (event: FormEvent) => {
    event.preventDefault();
    if (!valid) return;
...
function ForwardWizard({ grants, initialGrantID, nodes, onClose, onComplete }: { grants: TunnelGrant[]; initialGrantID?: string; nodes: NodeItem[]; onClose: () => void; onComplete: () => Promise<void> }) {
  ...
  const [draft, setDraft] = useState<ForwardDraft>({ grantID: grants.some((grant) => sameID(resourceID(grant), initialGrantID)) ? initialGrantID || "" : grants[0] ? String(resourceID(grants[0])) : "", targetNodeID: nodes[0] ? String(nodes[0].id) : "", name: "", sourceCIDRs: "" });
  const [preflight, setPreflight] = useState<PreflightResult | null>(null);
  const [working, setWorking] = useState<"preflight" | "create" | "">("");
  const [error, setError] = useState("");
  ...
  const stepValid = step === 1 ? Boolean(grant) : step === 2 ? Boolean(target) : step === 3 ? Boolean(draft.name.trim()) : Boolean(preflight && preflight.success !== false && preflight.ready !== false);
  const payload = () => ({ grant_id: draft.grantID, name: draft.name.trim(), target: { type: "managed_node", node_id: Number(draft.targetNodeID) }, network: "tcp", source_cidrs: sourceCIDRs });
  ...
  return <Dialog title="创建用户转发" description="首版使用固定 TCP、自动端口和受管节点目标。" onClose={onClose} wide dismissible={!working}><div className="fm-wizard"><ol className="fm-steps" aria-label="创建转发步骤">{([[1, "选择隧道"], [2, "选择目标"], [3, "转发设置"], [4, "确认创建"]] as const).map(([number, label]) => <li key={number} className={step === number ? "is-active" : step > number ? "is-done" : ""}><span>{step > number ? <Check size={14} /> : number}</span><strong>{label}</strong></li>)}</ol>{error ? <ErrorState message={error} /> : null}
    <div className="fm-wizard-content">{step === 1 ? <div className="fm-choice-grid">{grants.map((item) => { const template = tunnelForGrant(item, []); return <button type="button" key={String(resourceID(item))} className={sameID(resourceID(item), draft.grantID) ? "is-active" : ""} onClick={() => update("grantID", String(resourceID(item)))}><span><Route size={19} /><strong>{tunnelName(item, [])}</strong></span>{template ? <RouteLine hops={routeHops(template).map((hop) => ({ name: hop.name }))} /> : null}<small>{item.per_forward_speed_mbps ? `${item.per_forward_speed_mbps} Mbps` : "不限速"} · {item.per_forward_connection_limit ? `${item.per_forward_connection_limit} 连接` : "不限连接"} · 至 {displayDate(item.expires_at)}</small></button>; })}</div> : null}
      ...
      {step === 3 ? <div className="fm-form"><div className="fm-form-grid"><Field label="转发名称"><input autoFocus value={draft.name} onChange={(event) => update("name", event.target.value)} placeholder="例如：香港入口到美国 Reality" /></Field><Field label="网络类型"><select value="tcp" disabled><option value="tcp">TCP</option><option disabled>UDP（暂未支持）</option><option disabled>TCP + UDP（暂未支持）</option></select></Field></div><Field label="入口来源 CIDR 白名单（可选）" hint="每行或逗号分隔；留空时遵循隧道模板策略"><textarea rows={4} value={draft.sourceCIDRs} onChange={(event) => update("sourceCIDRs", event.target.value)} placeholder="例如：203.0.113.8/32" /></Field><div className="fm-capability-note"><ShieldCheck size={17} /><span>所有中间跳仅接受上一跳服务器流量；用户无法读取或修改内部端口。</span></div></div> : null}
      {step === 4 ? <div className="fm-review"><div className="fm-review-route"><span>固定路线</span>{grantTunnel ? <RouteLine hops={[...routeHops(grantTunnel).map((hop) => ({ name: hop.name })), { name: target?.node_name || "目标节点" }]} /> : <strong>{grant ? tunnelName(grant, []) : "已授权隧道"} → {target?.node_name}</strong>}</div><dl><div><dt>转发名称</dt><dd>{draft.name}</dd></div><div><dt>网络与入口</dt><dd>TCP · 自动分配端口</dd></div><div><dt>最终目标</dt><dd>{target?.node_name}</dd></div><div><dt>有效期</dt><dd>{displayDate(preflight?.effective_expires_at || grant?.expires_at)}</dd></div><div><dt>每转发限速</dt><dd>{grant?.per_forward_speed_mbps ? `${grant.per_forward_speed_mbps} Mbps` : "不限"}</dd></div><div><dt>连接数</dt><dd>{grant?.per_forward_connection_limit || "不限"}</dd></div><div><dt>计费流量</dt><dd>仅入口统计一次</dd></div><div><dt>入口预览</dt><dd>{preflight?.entry_host || preflight?.entry_address ? `${preflight.entry_host || preflight.entry_address}:${preflight.entry_port || "自动"}` : "创建后由系统分配"}</dd></div></dl><div className={`fm-preflight ${preflight?.success === false || preflight?.ready === false ? "is-bad" : "is-good"}`}><ShieldCheck size={18} /><span><strong>{preflight?.success === false || preflight?.ready === false ? "预检未通过" : "转发预检通过"}</strong><small>{preflight?.message || "授权、目标、路线和端口资源均满足创建条件"}</small>{preflight?.warnings?.map((warning) => <small key={warning}>{warning}</small>)}</span></div></div> : null}</div>
